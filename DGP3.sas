/************************************DGP 3: Multicollinearity with outliers*******************************/
proc iml;
...
do j = 1 to N_loop; 
*Define a correlation matrix that has Toeplitz form and definitely positive;
C = toeplitz ({1 0.5 0.3 0.8 0.7});

X1=j(N,1,0);
do t=1 to nrow(X1);
u=uniform(0);
if u < 0.8 then X1[t] = normal(0);
else X1[t]=normal(0)+4;
end;

X2=j(N,1,0);
do t=1 to nrow(X1);
u=uniform(0);
if u < 0.9 then X2[t] = normal(0);
else X2[t]=normal(0)+3;
end;

X3=j(N,1,0);
do t=1 to nrow(X1);
u=uniform(0);
if u < 0.95 then X3[t] = normal(0);
else X3[t]=normal(0)+2;
end;

X4=j(N,1,0);
do t=1 to nrow(X1);
u=uniform(0);
if u < 0.85 then X4[t] = normal(0);
else X4[t]=normal(0)+3;
end;

X5=j(N,1,0);
do t=1 to nrow(X1);
u=uniform(0);
if u < 0.9 then X5[t] = normal(0);
else X5[t]=normal(0)+ 3.5;
end;

Xvar = X1||X2||X3||X4||X5;

   S = J(nrow(Xvar), ncol(Xvar));
   /* T1: Create normal scores of each column */
   do i = 1 to ncol(Xvar);
      ranks = ranktie(Xvar[,i], "mean");          /* tied ranks */
   ...
   do i = 1 to ncol(Y);
      ...
      tmp = X_corr[,i]; call sort(tmp); /* sort column by ranks */
      X_corr[,i] = tmp[rank];           /* reorder the column of X by the ranks of M */
   end;

*Generate 45 variables non-corelated;
...
* Ceate a dataset for DPG3;
create myDataset var colNames;
...
submit;
        ...
        model y=X1-X50 / selection=forward (stop=AICC choose=AICC);
        run;
        ...
        model y=X1-X50 / selection=backward (stop=AICC choose=AICC);
        run;
        ...
        model y=X1-X50 / selection=stepwise (stop=AICC choose=AICC);
        run;
        ...
        model y=X1-X50 /  selection= LAR (stop=AICC  choose=AICC );
        run;
        ...
        model y=X1-X50 / selection=LASSO (stop=AICC  choose=AICC );
        run;
        ...
        model y=X1-X50 / selection=ELASTICNET (stop=AICC choose=AICC);
        run;
...
create myDataset var colNames;
   ...
   close myDataset;
        ...
        submit;
        proc glmselect noprint data=myDataset outdesign = DGP_results_forward ;
        model y=X1-X50 / selection=forward (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_forward out=DGP_results_for_trans;
		run;
  	
        proc glmselect noprint data=myDataset outdesign = DGP_results_backward;
        model y=X1-X50 / selection=backward (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_backward out=DGP_results_back_trans;
		run;
   
        proc glmselect noprint data=myDataset outdesign = DGP_results_step;
        model y=X1-X50 / selection=stepwise (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_step out=DGP_results_step_trans;
		run;
    
        proc glmselect noprint data=myDataset outdesign = DGP_results_lar;
        model y=X1-X50 /  selection= LAR (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_lar out=DGP_results_lar_trans;
		run;
    
        proc glmselect noprint data=myDataset outdesign = DGP_results_lasso;
        model y=X1-X50 / selection=LASSO (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_lasso out=DGP_results_lasso_trans;
		run;
    
        proc glmselect noprint data=myDataset outdesign = DGP_results_elast;
        model y=X1-X50 / selection=ELASTICNET (stop = CP choose=CP);
        run;
		proc transpose data=DGP_results_elast out=DGP_results_elast_trans;
		run;
    endsubmit;
...
endsubmit;
	...
	/************** Leave one out ***************************/
	/* The STOP=PRESS and CHOOSE=PRESS suboptions cannot be used with SELECTION=LAR, SELECTION=LASSO 
