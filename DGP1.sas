...
*Gaussian and independent data;
*multivariate normal distribution, where each is a centred reduced normal;
mean = j(nb_var,1,0);
x = randnormal(N, mean, I(nb_var));
...
* Create a dataset for DPG1;
    ...
    submit;
        ...
        proc glmselect noprint data=myDataset outdesign = DGP_results_lar;
        model y=X1-X50 /  selection= LAR (choose = sbc stop = PRESS LSCOEFFS);
        run;
		proc transpose data=DGP_results_lar out=DGP_results_lar_trans;
		run;
...
/************** stop = SBC choose = PRESS LSCOEFFS ***************************/
N_loop = 1000;
N = 500;
nb_var = 50;
beta = {0.4 1.2 0.9 0.2 1.6};
